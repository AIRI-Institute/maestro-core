...
from frontend_telegram.config import AuthConfig
from frontend_telegram.io_csv import get_values_from_csv, touch_csv, validate_csv_header, add_row_to_csv
...
class AuthManager:
    def __init__(self, config: AuthConfig):
        users_disabled = config.disabled or not config.whitelist_path
        admins_disabled = not bool(config.admin_path)

        self.users_white = SET_CACHE_ALWAYS_YES if users_disabled else UsersCache(config.whitelist_path)
        self.users_admins = SET_CACHE_ALWAYS_NO if admins_disabled else UsersCache(config.admin_path)
