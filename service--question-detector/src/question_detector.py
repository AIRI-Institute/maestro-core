...
import numpy as np
from mmar_mapi.api import BinaryClassifiersAPI, LLMAccessorAPI
from mmar_ptag import ptag_client
from loguru import logger
from sklearn.ensemble import HistGradientBoostingClassifier

from src.config import Config
from src.models import FirstWordCheck, GigaBasicCheck, GigaEmbedding, QuestionMarkCheck
...
class QuestionDetector(BinaryClassifiersAPI):
    def __init__(self, config: Config):
        llm_accessor = ptag_client(LLMAccessorAPI, config.llm.address)

        self.giga_embedding = GigaEmbedding(config, llm_accessor)
        self.question_mark = QuestionMarkCheck()
        self.giga_basic = GigaBasicCheck(config, llm_accessor)
